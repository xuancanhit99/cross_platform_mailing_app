...
class WebserviceSendMassEmail extends StatefulWidget {
  const WebserviceSendMassEmail({super.key});

  @override
  State<WebserviceSendMassEmail> createState() => _WebserviceSendMassEmailState();
}

class _WebserviceSendMassEmailState extends State<WebserviceSendMassEmail> {
  final _formKey = GlobalKey<FormState>();
  final senderNameController = TextEditingController(text: 'Xuan Canh IT');
  final senderEmailController = TextEditingController(text: 'xuancanhit99@gmail.com');
  final senderPasswordController = TextEditingController(text: 'kpmyefhtgsfmizjv');
  final subjectController = TextEditingController(text: 'Test Email');
  final messageController = TextEditingController(text: 'Hello, this is a test email');
  final webserviceURLController = TextEditingController(text: 'http://localhost:5000/send_email');
...
// Function to send mass emails
  void sendMassEmails(List<List<dynamic>> csvData) {
    for (List<dynamic> row in csvData) {
      ...
      print(attachments.map((file) => file.path).toList());
      _smtpConnectBloc.add(SmtpSendEmailViaURLPostEvent(
        webserviceUrl: webserviceURLController.text,
    ...
    return BlocProvider.value(
      ...
      child: BlocListener<SmtpConnectBloc, SmtpConnectState>(
        ...
        child: SafeArea(
            child: Scaffold(
          appBar: AppBar(
            title: const Text('Webservice Send Mass Email',
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          // Webservice URL
                          TextFormField(
                            controller: webserviceURLController,
                            keyboardType: TextInputType.url,
                            decoration: const InputDecoration(
                              prefixIcon: Icon(Icons.link),
                              labelText: 'Webservice URL',
                            ),
                            validator: (value) {
                              if (value!.isEmpty) {
                                return 'Please enter webservice URL';
                              }
                              return null;
                            },
                          ),
